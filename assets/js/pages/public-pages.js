  ...
  App.Pages.landing = function (root) {
    App.MockApi.getLandingData().then(function (data) {
      root.innerHTML = [
        ...
        "      </div>",
        '      <p class="mt-6 text-sm text-slateText">אין צורך להתחבר מראש. מזינים את פרטי המשכנתא באשף, ובשלב הסיום יוצרים את החשבון ושומרים את התוצאות.</p>',
        "    </div>",
        '    <div class="content-card rounded-[32px] border border-line p-6">',
        '      <h2 class="text-3xl font-extrabold text-ink">כלי החלטה, לא דף מכירה</h2>',
        '      <div class="mt-6 space-y-4">',
        '        <div class="rounded-[24px] border border-line bg-white px-5 py-4"><div class="flex items-center gap-3"><span class="icon-chip">' + icon("action") + '</span><div><p class="text-sm font-bold text-ink">התחלה חינמית</p><p class="text-sm text-slateText">האשף זמין ללא התחברות מוקדמת וללא עלות.</p></div></div></div>',
        '        <div class="rounded-[24px] border border-line bg-white px-5 py-4"><div class="flex items-center gap-3"><span class="icon-chip">' + icon("research") + '</span><div><p class="text-sm font-bold text-ink">גישה מחקרית</p><p class="text-sm text-slateText">פיתוח מבוסס כלכלה, מתמטיקה וניתוח תרחישים.</p></div></div></div>',
        '        <div class="rounded-[24px] border border-line bg-white px-5 py-4"><div class="flex items-center gap-3"><span class="icon-chip">' + icon("shield") + '</span><div><p class="text-sm font-bold text-ink">אישור לקוח לפני המשך טיפול</p><p class="text-sm text-slateText">אם נמצאה אפשרות טובה יותר, מבקשים לאשר עניין לפני כל המשך.</p></div></div></div>',
        "      </div>",
        "    </div>",
        "  </div>",
        "</section>",
