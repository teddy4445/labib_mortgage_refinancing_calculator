...
def calculate_advisor_fee(
    *,
    ...
    if default_amount < 0:
        raise ValidationError("Advisor-fee default amount cannot be negative.")
    if allocation_ratio <= 0:
        raise ValidationError("Advisor-fee allocation ratio must be greater than zero.")
    if user_quote is not None and user_quote < 0:
        raise ValidationError("Advisor-fee override cannot be negative.")
